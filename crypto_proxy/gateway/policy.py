...
UI_PREFIXES = (
    ...
    "/fallback/login",
    "/onboarding/",
    # 아래 둘은 **실브라우저 전수**(2026-08-31, 관리 UI 8화면)에서 403으로 잡혔다. 라우트 분류에
    # 같은 계열이 여럿 등재돼 있는데(`/get/ui_settings`·`/public/model_hub` 등) 일부만 빠져 있어,
    # 개별 경로가 아니라 계열로 잡는다. **데이터 플레인이 이 아래에 하나도 없음을 확인했다** —
    # `llm_api_routes`에 `/get/`·`/public/` 시작 경로가 0건이다(실측).
    "/get/",
    "/public/",
)
